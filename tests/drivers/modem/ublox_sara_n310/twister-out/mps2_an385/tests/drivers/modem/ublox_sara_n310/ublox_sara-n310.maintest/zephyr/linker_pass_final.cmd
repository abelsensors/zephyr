...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x0 + 0x0), LENGTH = (4096*1K - 0x0)
    SRAM (wx) : ORIGIN = 0x20000000, LENGTH = (4096 * 1K)
    IDT_LIST (wx) : ORIGIN = (0x20000000 + (4096 * 1K)), LENGTH = 2K
    }
ENTRY("__start")
SECTIONS
    {
 ...
 *(".kernel.*")
 ...
 net_if_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_if_list_start = .; KEEP(*(SORT_BY_NAME(._net_if.static.*))); _net_if_list_end = .; } > SRAM AT> FLASH
 net_if_dev_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_if_dev_list_start = .; KEEP(*(SORT_BY_NAME(._net_if_dev.static.*))); _net_if_dev_list_end = .; } > SRAM AT> FLASH
 net_l2_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_l2_list_start = .; KEEP(*(SORT_BY_NAME(._net_l2.static.*))); _net_l2_list_end = .; } > SRAM AT> FLASH
    __data_ram_end = .;
