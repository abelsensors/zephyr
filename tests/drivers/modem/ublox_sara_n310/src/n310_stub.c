				 ...
				 struct k_sem *sem, k_timeout_t timeout)
{
	...
	for (i = 0; i < cmds_len; i++) {
		...
		if (cmds[i].handle_cmd.cmd && cmds[i].handle_cmd.func) {
			...
			ret = modem_cmd_send(iface, handler, NULL, 0,
					     cmds[i].send_cmd, sem, timeout);
		}
...
extern int create_socket(struct modem_socket *sock,
...
extern int offload_connect(void *obj, const struct sockaddr *addr,
			   socklen_t addrlen);
